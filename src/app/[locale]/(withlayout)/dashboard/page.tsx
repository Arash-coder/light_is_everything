...
interface IMemberItem extends React.DetailedHTMLProps<React.HTMLAttributes<HTMLDivElement>, HTMLDivElement> {
  name: string;
  career: string;
  description: string;
...
const MemberItem = ({
  ...
  return (
    <div {...props}>
      ...
      <div className={`bg-primary text-center rounded-xl break-words overflow-hidden`}>
        <div className='p-4'>
          <Image className='w-32 h-32 object-contain m-auto mb-2' src={profilePicture} alt='profile-2' />
        ...
        <Link href={"#"} className='p-3 bg-secondary text-primary flex justify-center items-center' >
          دیدن پروفایل
        </Link>
      </div>
    </div>
  ...
  return (
    <div className='px-10  '>
      ...
      <div className='flex items-center w-full rounded-lg p-2  bg-primary  mb-7 '>
        ...
        <input placeholder='نام کاربر را جستجو کنید' className='outline-none w-full px-2' />
      </div>
      ...
      <div className='py-4 text-center !font-aria_sbold'>
        <StyledPagination locale={{ items_per_page: "/ صفحه" }}
          className='!font-aria_sbold' total={200} pageSize={10} />
