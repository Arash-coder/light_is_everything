...
interface ButtonProps extends React.DetailedHTMLProps<React.HtmlHTMLAttributes<HTMLButtonElement>, HTMLButtonElement> {
  focused?: boolean; disabled?: boolean; text: string; icon: JSX.Element;
}
const Button = ({ focused = false, disabled = false, text, icon, ...props }: ButtonProps) => {
  return (
    <button {...props} disabled={disabled} className={`${(!focused || disabled) && " ghost"} ${disabled && 'disabled'} btn block w-full ${props.className}`} >
    ...
    <div className='custom_container flex gap-3'>
      <div className='p-8 bg-neutral-200 rounded-xl py-12'>
        ...
        <Button
          ...
          text='گالری'
        />
        <Button disabled icon={<FaWallet size={20} />} text='سفارسات (به زودی)' />
